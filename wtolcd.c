...
//lcd操作
    ...
    while(1)
    {
        ...
        if(shmPtr->wtolcd.b_wtolcd_running == false)
        {
            sem_wait(&shmPtr->wtolcd.sem_wtolcd_wakeup);//睡眠等待控制台允许
        }

        sem_wait(&shmPtr->wtolcd.sem_wtolcd_standby);//等待获得写入lcd的信号量
	 	...
	 	while (cinfo.output_scanline < cinfo.output_height)
	 	{
	 		...
	 		if (lcdDev.lcdBpp == 16)
	 		{
	 			...
	 			for (x = 0; x < cinfo.output_width; x++)
	 			{
	 				...
	 				lcdFillPix(lcdDev.lcdMem, lcdDev.lcdBufWidth, lcdDev.lcdBufHeight, x+40, y+120, color);
	 			}
	 		}
