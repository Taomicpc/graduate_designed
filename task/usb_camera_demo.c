...
int main(int argc, char** argv)
{
	...
	int fd = open("/dev/video2", O_RDWR, 0);			//打开摄像头设备，使用阻塞方式打开
	if (fd<0)
	{
		printf("open error\n");
		return  -1;
	...
	if (ioctl(fd, VIDIOC_REQBUFS, &req) == -1)			//申请帧缓冲
	{
...
//	printf("sizeof(VideoBuffer) is %d\n", sizeof(VideoBuffer));
	...
	for (numBufs = 0; numBufs < req.count; numBufs++)
	{
		...
		buf.index = numBufs;//每一帧此信息不同
		if (ioctl(fd, VIDIOC_QUERYBUF, &buf) < 0)		//根据上面填充信息，获取缓冲帧的地址
		{
...
//		printf("pixelformat = %d\n", fmt.fmt.pix.pixelformat);
	...
	fb_munmap(fbdev.fb_mem, fbdev.fb_size);	//采用内存映射方式，关闭设备前需要释放framebuffer映射
